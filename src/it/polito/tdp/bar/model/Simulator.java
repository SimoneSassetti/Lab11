...
public class Simulator {
	...
	public void setGruppi(){
		...
		for(int i=0; i<2000;i++){
			...
			durata=(int)(60+(Math.random()*60+1));
	...
	private void setIN(Event e){
		...
		for(Tavolo t: listaTavoli){
			if(!trovato && t.isLibero() && 50*t.getNumPosti()/100<=e.getGruppo().getNumPersone() && t.getNumPosti()<=e.getGruppo().getNumPersone()){
				...
				numTotClienti=numTotClienti+e.getGruppo().getNumPersone();//DOVREI FARE DEI SET NELLA CLASSE STATISTICHE OGNI VOLTA
	...
	private boolean disponibilitaAndareBancone(float toll) {
		...
		//AFFINCHE I CLIENTI ACCETTINO DI ANDARE AL BANCONE LA PROBABILITA DEVE ESSERE CONTENUTA NELL INTERVALLO TRA 0 E LA TOLLERANZA, MENTRE CI SARA UN PICCOLA PARTE CHE
